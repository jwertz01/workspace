...
public class CandidateSpectrumGenerator {
     ...
     * @param outfile
     */
    public void generateIndexedLibrary(String outfile, double fragmentTolerance){
     ...
     * @param outfile
     */
    public void generateIndexedLibraryFromSorted(String outfile, double fragmentTolerance){
        this.table = new LargeHashMap(outfile);
...
//      }
    ...
    public static String indexSpectralLibrary(String libraryFile, double tolerance){
        ...
        if(!(new File(indexedFile).exists())){
            ...
            if(libraryFile.endsWith("mgf")){
                gen.generateIndexedLibrary(indexedFile, tolerance);
            }else{
                gen.generateIndexedLibrary(indexedFile, tolerance);
            }
        }else{
     ...
     * @param indexedFile
     */
    public static void indexSpectralLibrary(String libraryFile, String indexedFile, double fragmentTolerance){
        CandidateSpectrumGenerator gen = new CandidateSpectrumGenerator(libraryFile);
        gen.generateIndexedLibrary(indexedFile, fragmentTolerance);
    }
    ...
    public static void main(String[] args){
        ...
        if(args.length != 3){
            System.out.println("java -Xmx3000M -cp MSPLIT.jar Spectrum.CandidateSpectrumGenerator <spectral library> <index File> <fragment tolerance>");
            return;
        }
        String libraryFile = args[0];
        String indexFile = args[1];
        double fragmentTolerance = Double.parseDouble(args[2]);
        indexSpectralLibrary(libraryFile, indexFile, fragmentTolerance);
    }
}   
