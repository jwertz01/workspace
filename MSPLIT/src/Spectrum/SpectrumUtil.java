...
public class SpectrumUtil {
    ...
    public static void analyzeMixtureSpectralMatches(double fragmentTolerance){
        ...
        try{
            ...
            while(currentLine != null){
                ...
                s2 = s2.toNormVector();
                s2.sqrtSpectrum();
                double alpha = m.alpha(s1, s2, fragmentTolerance);
                System.out.println("Mathicng: " + m.peptide + " to " + s1.peptide + " and " + s2.peptide + " has similarity: " + m.maxScore(s1, s2, alpha, fragmentTolerance)
                        + "\t" + m.cosineSim(s1, fragmentTolerance) + "\t" + m.cosineSim(s2, fragmentTolerance) + "\t" + s1.cosineSim(s2, fragmentTolerance) + "\t" + alpha + "\t" + m.residual(s1, fragmentTolerance));
...
//      LMixturePeakScoreLearner learner = new LMixturePeakScoreLearner(trainingFile);
...
//  
    ...
    public static void loadAnnotationFromFile(String filename, List<Spectrum> specList){
        ...
        for(Iterator<Spectrum> it = specList.iterator(); it.hasNext();){
            Spectrum s = it.next();
            if(annotationMap.containsKey(s.spectrumName)){
                String annotation = annotationMap.get(s.spectrumName);
                String[] peps = annotation.split(" & ");
                //LinkedPeptide lp = new LinkedPeptide(peps[0]+"--"+peps[1], s.charge,  3, 6);
                s.setPeptide(annotation);//+"."+s.charge);
...
//      String libFile = "..\\mixture_linked\\linked_peptide_library\\sumo_lib\\20101008_Sumo_Library_4349_Bo.mzXML";
//      String annotationFile = "..\\mixture_linked\\test.txt";
//      String outfile = "..\\mixture_linked\\lib_sumo1_spectra_wrongTopIDCase.mgf";
